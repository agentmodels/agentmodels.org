...
#### Model that includes discounting: infer discounting behavior, preferences, softmax

Naive case: the low noise, naive scenario is a contender but MAP explanation is in terms of high softmax noise. This is reduced if we allow the agent to "back track" and return to their previous location. This is an unusual "mistake" for humans to make and so we rule it out. However, while only a single error can lead to the Naive trajectory, if we see the agent do this multiple times then noise becomes a much worse explanation. Inference after seeing agent do the same thing three times (last argument to getPosterior is 3).  
