...
### Greedy Planning
- One reason optimal planning is hard is that it has to take into account the entire time horizon. The (PO)MDP agent we described essentially reasons backwards from its actions on the last timestep. It thinks about what it would do then and then reasons back about how to get into good states. (With an infinite time horizon, the agent has to consider what it would do in every possible state, including states that can only be reached after a very long duration). If we applied this algorithm against a predictable chess opponent, it would consider every possible game we could play in some finite duration, where most of the games are incredibly unlikely. 

- Obvious alternative: instead of taking into account the entire possible future when taking an action, you only take into account the short-term. For example, in a problem that lasts $$1000$$ timesteps, you take your first action by optimizing for the first $$10$$ timesteps. For your second action, you recompute your plan, optimizing for timesteps $$2-11$$, and so on. Whereas the optimal agent computes a complete plan (i.e. *policy*) before the first timestep and does no more computation after that, the "greedy" alternative involves computing a short-term plan at every timestep. Spreading out the computation in this way can be much more tractable than the optimal approach. The Greedy approach only considers states or belief-states that it actually enters (or *could* enter in the short-term), while the Optimal approach considers every possible (belief) state.

The Greedy approach will work best if continually optimizing for the short-term produces good results in the long-term. It's easy to imagine probems where this doesn't hold. For example, if you can only get a big prize by going down a very long corridor (which provides no rewards along the way), then being Greedy will fail. One remedy is to find a proxy for the long-term expected utility of being in a state that is cheap to compute. For example, in games like chess, we might be Greedily optimize the score of future chess positions using a cheaply computed evaluation function. In this chapter, we 




For instance, instead of taking into 1000 timesteps into account you only consider how well you'll do on the next 10. 


and then revise that plan continually. The idea is that if, at each time step, you take actions are good in the short-term, you will do well in the long-term. Clearly, there are environments where lack of long-term planning will lead to poor long-term results. However, 
