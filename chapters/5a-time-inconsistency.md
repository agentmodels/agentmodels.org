...
[^exponential]: There are arguments that exponential discounting is the uniquely rational mode of discounting for agents with time preference. See X and Y for critical discussion (toby, dasgupta, wolfgang schwarz). Strotz 1955: "In the continuous time setting, the only discount function such that the optimal policy doesn't vary in time is exponential discounting". Lattimore and Hutter 2014 proves the same result in the discrete setting, as well as discussing optimal strategies for sophisticated time-inconsistent agents.
